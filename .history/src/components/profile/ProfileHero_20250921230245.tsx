  ...
  return (
    <section className="hero" aria-label={`${name} profile header`}>
      ...
      <div className="copy">
        ...
        {(email || linkedinHref) && (
          <div className="actions" role="list" aria-label="Contact">
            {email && (
              <Link href={`mailto:${email}`} className="chip" role="listitem" aria-label="Contact">
                <MailIcon />
                <span>Contact</span>
              </Link>
            )}
            {linkedinHref && (
              <Link href={linkedinHref} className="chip" role="listitem" aria-label="LinkedIn">
                <LinkedInIcon />
                <span>LinkedIn</span>
              </Link>
            )}
      ...
      <style jsx>{`
        ...
        @media (prefers-color-scheme: dark) {
          .hero { --surface: #0e0f12; }
          .title, .bio { color: rgba(255, 255, 255, 0.85); }
          .chip { background: #1f2937; }
          .location { color: #60a5fa; }
          .avatar { box-shadow: 0 0 0 6px #0e0f12, 0 2px 30px rgba(0,0,0,0.35); }
        }
      `}</style>
    </section>
  );
