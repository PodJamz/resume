...
export default function CursorIDEHero({ className = "" }: CursorIDEHeroProps) {
  ...
  return (
    <div className={`w-full h-full bg-gradient-to-br from-purple-600 via-pink-500 to-blue-500 rounded-2xl overflow-hidden shadow-2xl ${className}`}>
      <div className="bg-gray-900 text-white h-full relative flex flex-col">
        {/* Window Controls */}
        ...
        <div className="flex flex-1 min-h-0">
          {/* Sidebar */}
          <div className="w-64 bg-gray-800 border-r border-gray-700 p-4 hidden lg:flex flex-col overflow-y-auto">
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            <div className="flex-1 p-4 overflow-auto">
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 min-h-full">
                {agents.map((agent, index) => {
                  ...
                  return (
                    <div
                      key={agent.id}
                      className={`bg-gray-800 rounded-lg border transition-all duration-300 hover:shadow-lg hover:scale-[1.02] flex flex-col h-fit ${
                        isCompleted
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between p-3 border-b border-gray-700 flex-shrink-0">
                        <div className="flex items-center space-x-2 min-w-0">
                          {isCompleted ? (
                            ...
                            <Loader2 className={`w-4 h-4 animate-spin flex-shrink-0 ${getColorClasses(agent.color).split(" ")[0]}`} />
                          )}
                          <IconComponent className={`w-4 h-4 flex-shrink-0 ${getColorClasses(agent.color).split(" ")[0]}`} />
                      ...
                      <div className="flex-1 p-3 space-y-3 min-h-0">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {(currentStep?.files || []).slice(0, 2).map((file, i) => (
                            <div
                              key={i}
                            ...
                            >
                              ...
                              <span className="text-white truncate max-w-[80px]">{file}</span>
                              {currentStep?.completed && <Check className="w-3 h-3 text-green-400 flex-shrink-0" />}
                            </div>
                          ))}
                          {(currentStep?.files || []).length > 2 && (
                            <div className="flex items-center space-x-1 bg-gray-700 rounded px-2 py-1 text-xs">
                              <span className="text-gray-400">+{(currentStep?.files || []).length - 2}</span>
                            </div>
                          )}
                        </div>

                        {/* Code Preview */}
                        <div className="bg-gray-900 rounded p-2 text-xs font-mono overflow-hidden">
                          <div className="max-h-24 overflow-y-auto">
                            {(currentStep?.code || []).slice(0, 8).map((line, i) => (
                              <div key={i} className="text-gray-300 leading-relaxed">
                                {line && (
                                  <>
                                    ...
                                    <span
                                      className={
                                        isTyping[agent.id] && i === Math.min(currentStep.code.length - 1, 7) ? "animate-pulse" : ""
                                      }
                                    >
                                      {line}
                                    </span>
                            ...
                            {(currentStep?.code || []).length > 8 && (
                              <div className="text-gray-500 text-center py-1">...</div>
                            )}
                          </div>
                          {isTyping[agent.id] && currentStep && !currentStep.completed && (
                            <div className="flex items-center space-x-1 mt-1">
                              <span className="text-gray-500 select-none">{Math.min(currentStep.code.length + 1, 9)}</span>
                      ...
                      {/* Agent Actions */}
                      <div className="p-3 border-t border-gray-700 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center space-x-2">
                          <button
              ...
              <div className="bg-gray-800 rounded-lg border border-gray-700 p-3">
                <div className="flex items-center space-x-3">
                  <div className="flex-1 bg-gray-700 rounded-lg px-3 py-2">
                    <div className="text-xs text-gray-400">Add context</div>
                    <div className="text-white text-sm">Create comprehensive startup assets...</div>
                  </div>
                  <button className="px-3 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-medium transition-colors flex-shrink-0">
                    Send
                  </button>
                </div>
