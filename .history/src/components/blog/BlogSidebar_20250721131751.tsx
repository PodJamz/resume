...
import ParadoxCubeIcon from "./ParadoxCubeIcon";
...
export const BlogSidebar: React.FC<BlogSidebarProps> = ({ items }) => {
  ...
  const renderPostIcon = (item: SidebarItem) => {
    switch (item.id) {
      case 'Cursor':
        return (
          <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center p-1">
            <Image src="/cursor.png" alt="Cursor" width={20} height={20} className="object-contain" />
          </div>
        );
      case 'hello-world':
        return <HalftoneWaveMini size={32} className="ring-2 ring-white/20" />;
      default:
        return (
          <span
