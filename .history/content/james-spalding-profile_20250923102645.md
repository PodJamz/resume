...
- I Build systems that stay evaluable end-to-end. Every feature is tracked in Linear (often via MCP hooks), tied to clear PRDs and scope. I use safe defaults in design so the first version always works without surprises, then layer in observability (dashboards, scorecards, error traces) to see what breaks. Rollback is always part of the plan — risky changes ship to a small slice first, so we can revert quickly.  
